...
function WeatherJunction(props) {
    ...
    function fetchForecastWeather(lat, lon) {
        ...
        return (async dispatch => {
            ...
            try {
                const currentSpaceData = await axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}.04&appid=9df3f65223f5d0da919ec90525134833`)
                debugger
                    ...
                    <Formik>
                        {() => (
                            <Form>
                                ...
                                <Button variant="contained" onClick={() => dispatch(fetchCurrentWeather(lat, lon))} fullWidth> GET WEATHER </Button>
                            </Form>
                        )}
                    </Formik>
                </Paper>
